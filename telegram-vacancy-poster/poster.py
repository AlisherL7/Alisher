...
async def send_to_group(client: TelegramClient, group: dict, rng: random.Random) -> str:
...
async def run_cycle(client: TelegramClient, rng: random.Random) -> None:
    ...
    for group in order:
        ...
        result = await send_to_group(client, group, rng)
        if result == "flood_stop":
            return

        pause = rng.uniform(config.POST_DELAY_MIN, config.POST_DELAY_MAX)
